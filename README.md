...
通常の会話表示だけでは、AIが応答を終了した時点で次周回は動きません。このプロジェクトでは、Codex CLIを一周ずつ継続起動するランナーを用意しています。ローカルサーバーは起動しません。

```bash
npm run harness:loop          # ユーザーが停止するまで周回を続ける
npm run harness:stop          # 現在の周回の完了後に停止を要求する
npm run harness:loop -- --resume  # 停止後に再開する
```

状態はローカルの `.harness/runtime.json` に保存され、Gitへは含めません。各周回は、調査・生成・辛口評価・記録・検証を終えてから次へ移ります。人の確認待ちは停止条件にしません。ランナーが失敗した時だけ停止し、状態を残します。
